          ...
          <div style={{ width: "100%", maxWidth: "none", margin: "0 auto" }}>
            ...
            {success ? (
              <div style={{ textAlign: "center", padding: "12px 0" }}>
                <div style={{ width: 60, height: 60, background: "rgba(0,132,61,0.15)", border: `2px solid ${C.green}`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px", animation: "checkPop 0.4s ease" }}>
                  <span style={{ fontSize: 26, color: C.white }}>✓</span>
                </div>
                <div style={{ fontWeight: 700, fontSize: 17, color: C.white, marginBottom: 8 }}>Password Updated!</div>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.65)", lineHeight: 1.6 }}>
                  Your password has been changed successfully.<br />Redirecting you to sign in...
                </div>
                <div style={{ marginTop: 18, display: "flex", justifyContent: "center" }}>
                  <div style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.2)", borderTop: `2px solid ${C.white}`, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.9)", display: "block", marginBottom: 6 }}>New Password</label>
                  <div style={{ position: "relative" }}>
                    <input style={{ ...inp, paddingRight: 48, borderColor: passwordTooShort ? "#fca5a5" : "rgba(255,255,255,0.15)" }} type={show.pw ? "text" : "password"} placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password"
                  ...
                  {password && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                        {[1, 2, 3, 4].map((i) => (
                          <div key={i} style={{ flex: 1, height: 4, borderRadius: 4, background: i <= pw.score ? pw.color : "rgba(255,255,255,0.14)", transition: "background 0.3s" }} />
                        ))}
                      </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.9)", display: "block", marginBottom: 6 }}>Confirm Password</label>
                  <div style={{ position: "relative" }}>
                    <input style={{ ...inp, paddingRight: 48, borderColor: confirm && !passwordsMatch ? "#fca5a5" : "rgba(255,255,255,0.15)" }} type={show.cf ? "text" : "password"} placeholder="Repeat your password" value={confirm} onChange={(e) => setConfirm(e.target.value)} autoComplete="new-password"
                ...
                <button type="submit" disabled={loading} style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", background: loading ? C.gray200 : C.green, color: C.white, fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: loading ? "none" : `0 4px 16px ${C.green}55` }}>
                  {loading ? (<><div style={{ width: 15, height: 15, border: "2px solid rgba(255,255,255,0.3)", borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Updating...</>) : "Update Password →"}
                </button>

                <button type="button" onClick={handleBack} style={{ width: "100%", padding: "13px", borderRadius: 12, marginTop: 12, border: "1.5px solid rgba(255,255,255,0.2)", background: "transparent", color: "rgba(255,255,255,0.7)", fontWeight: 600, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}
        ...
        <div style={{ position: "relative", zIndex: 1, background: C.white, borderRadius: 20, padding: "40px 36px", width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.35)", animation: "fadeIn 0.35s ease" }}>
          ...
          {success ? (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              <div style={{ width: 64, height: 64, background: `${C.green}15`, border: `2px solid ${C.green}`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", animation: "checkPop 0.4s ease" }}>
                <span style={{ fontSize: 28 }}>✓</span>
              </div>
              <div style={{ fontWeight: 700, fontSize: 16, color: C.text, marginBottom: 8 }}>Password Updated!</div>
              <div style={{ fontSize: 14, color: C.gray400, lineHeight: 1.6 }}>Your password has been changed successfully.<br />Redirecting you to sign in...</div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                {password && (
                  <div style={{ marginTop: 8 }}>
                    ...
                    {pw.label && <div style={{ fontSize: 12, color: pw.color, fontWeight: 600 }}>{pw.label} password</div>}
                  </div>
                )}
              </div>
              ...
              <button type="submit" disabled={loading} style={{ width: "100%", padding: "13px", borderRadius: 10, border: "none", background: loading ? C.gray200 : C.green, color: C.white, fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", transition: "background 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                {loading ? (<><div style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.3)", borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Updating...</>) : "Update Password →"}
              </button>

              <button type="button" onClick={handleBack} style={{ width: "100%", padding: "11px", borderRadius: 10, marginTop: 12, border: `1.5px solid ${C.gray200}`, background: C.white, color: C.gray400, fontWeight: 600, fontSize: 14, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}
