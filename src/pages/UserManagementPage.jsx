...
function ChangeRoleModal({ user, roles, callerRole, onClose, onSave, showToast }) {
  ...
  return (
    <Modal
      title="Change Role"
      subtitle={`Assigning to ${user.full_name || "user"}`}
      onClose={onClose}
...
function InviteModal({ roles, callerRole, callerCds, onClose, onSuccess, showToast }) {
  ...
  }, [form, onClose, onSuccess, showToast]);
        ...
        {form.password.length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
            {passwordChecks.map(c => (
              <span
                key={c.label}
                style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 5, background: c.ok ? "#dcfce7" : "#fee2e2", color: c.ok ? "#166534" : "#991b1b" }}
              >
                {c.ok ? "✓" : "✗"} {c.label}
              </span>
            ))}
...
export default function UserManagementPage({ role, showToast, profile }) {
  ...
  const stats = useMemo(() => {
    ...
    const rCounts = Object.fromEntries(
      Object.keys(ROLE_META).map(c => [c, users.filter(u => u.role_code === c).length])
    );
    ...
    <div style={{ height: "calc(100vh - 118px)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10, flexShrink: 0, alignItems: "center", flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
          ...
          <input
            placeholder="Search by name, CDS or phone..."
            value={search}
              ...
              >
                ...
                <div style={{ display: "flex", gap: 5 }}>
                  <button
                    ...
                    style={{ padding: "4px 9px", borderRadius: 7, border: `1px solid ${C.gray200}`, background: C.white, color: C.text, fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", transition: "all 0.12s" }}
                  ...
                  {user.role_code && (
                    <button
                      ...
                      style={{ padding: "4px 9px", borderRadius: 7, border: "none", background: user.is_active ? "#fef2f2" : "#f0fdf4", color: user.is_active ? "#dc2626" : "#16a34a", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
