...
export default function SystemSettingsPage({ role, session, showToast, setLoginSettings, companies, setCompanies, transactions }) {
  const [activeMenu, setActiveMenu] = useState(() => {
    try {
      return localStorage.getItem(ACTIVE_MENU_KEY) || "companies";
    } catch {
      return "companies";
    }
  });
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                      ...
                      <div style={{ marginTop: 16 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          {COLOR_PRESETS.map(p => (
                            <button
                              key={p.value}
                              onClick={() => setSlideField(idx, "color", p.value)}
                              title={p.label}
                    ...
                    <div>
                      ...
                      <Field label="Label (small gold text)">
                        <input
                          style={inp()}
                          placeholder="e.g. Investors Portal"
                          value={slide.label || ""}
                      ...
                      <Field label="Title (large white text)">
                        <input
                          style={inp()}
                          placeholder="e.g. Secure Investing"
                          value={slide.title || ""}
