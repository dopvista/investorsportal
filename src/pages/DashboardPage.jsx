...
function DonutChart({ segments, size = 130, thickness = 24 }) {
  ...
  // Build segments — start at top (12 o'clock = offset by circ/4)
  let cumulativeDash = 0;
  const paths = segments.map((seg, i) => {
    const pct  = seg.value / total;
    const dash = pct * circ;
    const gap  = circ - dash;
    // offset = circ/4 (start at top) - sum of previous dashes
...
export default function DashboardPage({ profile, role, session, showToast, onNavigate }) {
  ...
  // Filter transactions to this user's CDS
  ...
  const metrics = useMemo(() => {
    ...
    // Per-company portfolio computation from verified transactions
    let totalValue    = 0;
    let totalInvested = 0;
    ...
    const companyMetrics = portfolio.map((company, idx) => {
      ...
      // Aggregate net shares and invested capital
      let netShares  = 0;
      let buyShares  = 0;
      let buyInvested = 0;
    ...
    // Add weight after total is known
  ...
  // Donut segments for status chart
  ...
  // Portfolio weight donut (only companies with value)
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "right" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: C.gold, letterSpacing: "0.05em" }}>
            {profile?.cds_number || "—"}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <StatCard
          ...
          label="Gain / Loss"
          value={loading ? "—" : (metrics.hasFinancials
            ? (metrics.totalGainLoss >= 0 ? "+" : "") + fmtShort(metrics.totalGainLoss)
            : "—"
          )}
          subLabel={metrics.hasFinancials
            ? `${metrics.totalReturnPct >= 0 ? "+" : ""}${metrics.totalReturnPct.toFixed(2)}% return`
            : "Set prices to compute"
          }
          accent={metrics.totalGainLoss >= 0 ? C.green : C.red}
          onClick={metrics.hasFinancials ? () => toggleExpand("gain") : undefined}
          active={expanded === "gain"}
          loading={loading}
        />
          ...
          {portfolio.length === 0 ? (
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {["Company", "Current Price", "Prev. Price", "Change", "Last Updated"].map(h => (
                      <Th key={h}>{h}</Th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {metrics.allPortfolio.map((c, i) => {
                    ...
                    return (
                      <tr key={c.id} style={{ borderBottom: `1px solid ${C.gray100}`, background: i % 2 ? C.gray50 + "60" : "transparent" }}>
                        ...
                        <Td bold color={c.currentPrice > 0 ? C.green : C.gray400}>
                          {c.currentPrice > 0 ? fmt(c.currentPrice) : "—"}
                        </Td>
                        <Td color={C.gray500}>{c.prevPrice > 0 ? fmt(c.prevPrice) : "—"}</Td>
                        <Td>
                          {change !== null ? (
                            <span style={{ color: change >= 0 ? C.green : C.red, fontWeight: 700, fontSize: 13 }}>
        ...
        <ExpandPanel title="💰 Portfolio Breakdown" onClose={() => setExpanded(null)}>
          {metrics.companyMetrics.length === 0 ? (
            <div style={{ textAlign: "center", color: C.gray400, fontSize: 13, padding: "24px 0" }}>
              {portfolio.length === 0
                ? "No companies in your portfolio yet."
                : "Set current prices in Portfolio page to see value breakdown."}
            </div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {["Company", "Net Shares", "Avg Cost", "Current Price", "Market Value", "Gain/Loss", "Return %", "Weight"].map(h => (
                      <Th key={h}>{h}</Th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {metrics.companyMetrics.map((c, i) => (
                    <tr key={c.id} style={{ borderBottom: `1px solid ${C.gray100}`, background: i % 2 ? C.gray50 + "60" : "transparent" }}>
                      ...
                      <Td bold color={c.gainLoss >= 0 ? C.green : C.red}>
                        {c.gainLoss !== 0 ? (c.gainLoss >= 0 ? "+" : "") + fmtShort(c.gainLoss) : "—"}
                      </Td>
                      <Td>
                        {c.returnPct !== 0 ? (
                          <span style={{
                            background: c.returnPct >= 0 ? "#f0fdf4" : "#fef2f2",
                            color: c.returnPct >= 0 ? C.green : C.red,
                      ...
                      <Td>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ width: 50, height: 5, background: C.gray100, borderRadius: 4, overflow: "hidden", flexShrink: 0 }}>
                            <div style={{ width: `${c.weight}%`, height: "100%", background: c.color, borderRadius: 4, transition: "width 0.5s ease" }} />
                          </div>
                ...
                <tfoot>
                  <tr style={{ borderTop: `2px solid ${C.gray200}`, background: C.gray50 }}>
                    ...
                    <td style={{ padding: "10px 12px" }}>
                      <span style={{
                        background: metrics.totalReturnPct >= 0 ? "#f0fdf4" : "#fef2f2",
                        color: metrics.totalReturnPct >= 0 ? C.green : C.red,
                        padding: "2px 8px", borderRadius: 8,
          ...
          {metrics.companyMetrics.length > 0 ? (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {["Company", "Invested (TZS)", "Market Value", "Gain / Loss", "Return %"].map(h => (
                      <Th key={h}>{h}</Th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {metrics.companyMetrics.map((c, i) => (
                    <tr key={c.id} style={{ borderBottom: `1px solid ${C.gray100}`, background: i % 2 ? C.gray50 + "60" : "transparent" }}>
                      ...
                      <Td bold color={c.gainLoss >= 0 ? C.green : C.red}>
                        {(c.gainLoss >= 0 ? "+" : "") + fmtShort(c.gainLoss)}
                      </Td>
                      <Td>
                        <span style={{
                          background: c.returnPct >= 0 ? "#f0fdf4" : "#fef2f2",
            ...
            <div style={{ textAlign: "center", color: C.gray400, fontSize: 13, padding: "16px 0" }}>
              Set prices in Portfolio page to see per-company breakdown.
            </div>
          )}
        </ExpandPanel>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: 16 }}>
        ...
        <div style={{ background: C.white, border: `1px solid ${C.gray200}`, borderRadius: 14, padding: "20px" }}>
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{ flexShrink: 0 }}>
                <DonutChart
                  segments={metrics.total > 0 ? statusSegments : [{ value: 1, color: C.gray100 }]}
                  size={130}
                  thickness={24}
                />
              </div>
              <div style={{ flex: 1 }}>
                {statusSegments.map(seg => (
                  <div key={seg.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 9 }}>
                    ...
                    <span style={{ fontWeight: 700, fontSize: 13, color: seg.value > 0 ? C.text : C.gray300 }}>
                      {seg.value}
                    </span>
                  </div>
                ))}
          ...
          {/* Portfolio weight donut — only if data exists */}
                ...
                <div style={{ flex: 1 }}>
                  {portfolioSegments.map(seg => (
                    <div key={seg.label} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                      ...
                      <span style={{ fontSize: 11, color: C.gray500, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {seg.label}
                      </span>
                      <span style={{ fontSize: 11, fontWeight: 700, color: C.gray500 }}>
                        {metrics.totalValue > 0 ? ((seg.value / metrics.totalValue) * 100).toFixed(1) : 0}%
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <div style={{ background: C.white, border: `1px solid ${C.gray200}`, borderRadius: 14, padding: "20px" }}>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: "center", color: C.gray400, fontSize: 13, padding: "40px 0" }}>
